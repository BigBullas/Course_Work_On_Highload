...
- Cassandra (реляционная база данных) будем использовать для оставшихся таблиц
...
### База данных
В качестве основной базы данных была выбрана Cassandra. И основным её конкурентом являлась MongoDB. Cassandra и MongoDB - это две популярные распределенные базы данных, которые предназначены для хранения больших объемов данных и обладают определенными характеристиками, делающими их подходящими для различных сценариев использования. После прочтения [данного источника](https://habr.com/ru/companies/otus/articles/690924/) можно понять, что данные БД примерно идентичны. Выбор же пал на Cassandra только потому, что она использует язык запросов CQL (Cassandra Query Language), который схож с SQL. Что не скажешь о Query Language, который нужен для MongoDB. И ещё одно преимущество Cassandra заключается в том, что она изначально сильно затачивалась под горизонтальное масштабирование. В связи с этим его делать на Cassandra проще, чем на MongoDB.
