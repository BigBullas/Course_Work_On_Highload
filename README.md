...
присутствует сжатие GZIP (что также важно для Pinterest)
...
SSL терминация - это процесс расшифровки зашифрованных HTTPS-запросов от клиентов на сервере перед их дальнейшей
обработкой. Это позволяет серверу прочитать содержимое запроса и взаимодействовать с ним без шифрования, что облегчает
анализ и обработку данных, а также может повысить производительность сервера. SSL терминация выполняется на 
балансировщике нагрузки, перед тем как запросы передаются на бэкэнд-серверы для дальнейшей обработки.